...
**Sweep-scale**: 32 regime windows (CPU plumbing). Bear-d and other Cohen's d
values here differ from the full-eval runs reported in COMPARISON_TABLE.md
and README.md. The sweep is ONLY used to show the monotonic trend across w;
headline effect sizes come from the full-eval runs `cfg_eval_w2` / `cfg_eval_w4`.
...
**Note:** This report was generated on CPU with limited test windows (32).
Full sweep requires GPU host — see HOST_TASKS.md. Headline bear-d values
from the full-eval runs are: w=2 → 0.872, w=4 → 1.206 (see COMPARISON_TABLE.md).

Source: `runs/cfg_sweep__20260713-205210__seed0/metrics/cfg_sweep.json`.
